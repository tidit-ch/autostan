// Iter 7: 3D correlated (alpha, beta, gamma) via LKJ + unit-specific sigma
data {
  int<lower=0> N_train;
  int<lower=0> N_test;
  int<lower=0> J;
...
model {
  mu[1] ~ normal(0, 5);
  mu[2] ~ normal(0, 5);
  mu[3] ~ normal(0, 2);   // quadratic term expected smaller
  tau[1] ~ exponential(1);
  tau[2] ~ exponential(1);
  tau[3] ~ exponential(2);  // tighter prior for quadratic spread
