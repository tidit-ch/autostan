// Iteration 6: NCP + heteroscedastic + Student-t + nu ~ gamma(2,0.2) (heavier tail emphasis)
// Iter 5 showed fixed nu=10 worse than estimated nu; prior gamma(2,0.1) mean=20; try gamma(2,0.2) mean=10
data {
  int<lower=0> N_train;
  int<lower=0> N_test;
  int<lower=0> J;
...
model {
  ...
  nu ~ gamma(2, 0.2);  // mean=10, sd=7: encourages heavier tails than gamma(2,0.1)
