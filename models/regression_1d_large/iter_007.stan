...
// Iteration 7: Student-t + learnable frequency omega + quadratic log-sigma
// Iter 5 best: sine basis with fixed omega=pi/3 + quadratic het., NLPD=1.2854.
// Adding iter 6 quadratic correction didn't help. Now freeing omega to be estimated.
// If true frequency differs from pi/3, this should improve the mean fit significantly.
...
parameters {
  ...
  real b1;
  real beta_lin;
  real<lower=0.3, upper=3.0> omega;  // frequency parameter, constrained around pi/3 ~ 1.047
  real s0;
  real s1;
  real s2;
  real<lower=1> nu;
...
model {
  ...
  b1 ~ normal(0, 5);
  beta_lin ~ normal(0, 5);
  omega ~ normal(pi() / 3.0, 0.3);  // centered on pi/3, moderate uncertainty
  s0 ~ normal(0, 2);
  s1 ~ normal(0, 1);
  s2 ~ normal(0, 0.5);
  nu ~ gamma(2, 0.1);
