...
// Iteration 10: Gaussian mixture with estimated sigma_out (tight prior) + learnable omega + quadratic het.
// Iter 9 NLPD=1.2325 used fixed sigma_out=10. Fixed value may not be optimal.
// Allowing sigma_out to be estimated with lognormal prior centered at 10.
// If optimal sigma_out != 10, this should improve NLPD. Tight prior to prevent label-switching.
...
parameters {
  ...
  real<lower=3> sigma_out;  // constrained > 3 to prevent label-switching with inlier sigma
}
...
model {
  ...
  s2 ~ normal(0, 0.5);
  pi_out ~ beta(1, 9);
  sigma_out ~ lognormal(log(10), 0.5);
