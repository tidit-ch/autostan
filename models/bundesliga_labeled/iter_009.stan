...
//
// Iteration 9: Team-specific home advantage (hierarchical NCP).
// delta[j] = delta_mu + sigma_delta * delta_raw[j]
// lambda_home = exp(mu + delta[home_team] + attack[home] - defense[away])
// Bundesliga teams differ in home support; some exploit home ground more.
// ~8-9 home games per team in training — weak signal, hierarchical pooling.
// Base: iter 6 model (best NLPD=1.5460), all else identical.
...
parameters {
  ...
  real<lower=0> sigma_delta;  // between-team variation in home advantage
  real mu;
  real<lower=0> sigma_att;
  real<lower=0> sigma_def;
  vector[J] delta_raw;    // NCP team-specific home advantage
  vector[J] attack_raw;
...
model {
  ...
  sigma_delta ~ normal(0, 0.3);  // expect small between-team variation in home adv
  mu ~ normal(0.35, 0.5);
