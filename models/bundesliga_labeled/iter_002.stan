...
//
// Iteration 2: Negative binomial likelihood with hierarchical priors.
// Football goals are often overdispersed relative to Poisson; NB adds
// a dispersion parameter phi to capture extra variance.
...
parameters {
  ...
  vector[J] attack;
  vector[J] defense;
  real<lower=0> phi;  // NB overdispersion: phi -> inf gives Poisson
}
...
model {
  ...
  for (n in 1:N_train) {
    ...
    goals_home_train[n] ~ neg_binomial_2(lambda_home, phi);
    goals_away_train[n] ~ neg_binomial_2(lambda_away, phi);
  }
}
...
generated quantities {
  ...
  for (n in 1:N_test) {
    ...
    log_lik[n] = neg_binomial_2_lpmf(goals_home_test[n] | lambda_home, phi);
    log_lik[N_test + n] = neg_binomial_2_lpmf(goals_away_test[n] | lambda_away, phi);
  }
}
